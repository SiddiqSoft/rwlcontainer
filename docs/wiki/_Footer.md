<!-- GitHub Wiki Footer -->

---

## About RWLContainer

**RWLContainer** is a C++20 library providing thread-safe container classes for concurrent programming.

### Key Features
- ✅ Thread-safe dictionary with reader-writer locking
- ✅ Thread-safe queue for producer-consumer patterns
- ✅ High performance with minimal overhead
- ✅ Comprehensive documentation and examples
- ✅ Full C++20 support

### Quick Links
- [GitHub Repository](https://github.com/SiddiqSoft/RWLContainer)
- [NuGet Package](https://www.nuget.org/packages/SiddiqSoft.RWLContainer/)
- [Issue Tracker](https://github.com/SiddiqSoft/RWLContainer/issues)

### Documentation
- [Home](Home)
- [Getting Started](Getting-Started)
- [API Reference](RWLContainer)
- [Examples](Examples)
- [FAQ](FAQ)

### Support
- **C++ Standard:** C++20 or later
- **Platforms:** Windows, Linux, macOS
- **License:** BSD 3-Clause

---

**Version:** 1.5.5  
**Last Updated:** 2024  
**Maintainer:** Siddiq Software LLC

© 2021-2024 Siddiq Software LLC. All rights reserved.
